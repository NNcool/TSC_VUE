            ...
            <el-scrollbar wrap-class="scroll-wrapper">
              <div class="content-container">
                ...
                <div>
                  <h3>当前点击的地区岩性列表：</h3>
                  <ul>
                    <li v-for="leaf in clickedLeaves" :key="leaf.id" @click="handleOutputClick(leaf)"
                      class="clickable-item">{{ leaf.lithologyPattern + "(" + leaf.topAge + ", " + leaf.age + ")" }}</li>
                  </ul>
                </div>
              </div>
            </el-scrollbar>
...
export default {
  ...
  methods: {
    ...
    getOutputs: function (data) {
      ...
      this.$axios.get("/output/findsByAreaId/" + data.id)
        .then((response) => {
    ...
    drawLine(id) {
      this.$axios.get("/output/findRangeSpotById/" + id)
        .then((response) => {
